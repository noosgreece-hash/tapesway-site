...
## Loading
A loading screen (the logo and a brass progress line) covers the page only until the first seconds of film are in: the first 6 atlases (preloaded from `index.html` for the matching screen size) and the first four decoded, plus the rest of the film if the measured speed says it would arrive within 4 seconds. It never waits more than 3.5 s, and the CSS failsafe hides it after 8 s even if the script fails. Behind it, atlases stream six at a time: the ones just ahead of the current frame first, then the rest in playback order, or coarse to fine (every 4th, then every 2nd) when the rest would take more than 6 s at the measured speed, so a fast scroll on a slow line still finds a nearby frame.

Throttled headless Chromium, time until the page is usable (before → after this change): desktop 50 Mbit/s 4.0 → 0.6 s; desktop 10 Mbit/s 12.3 → 4.0 s; phone 20 Mbit/s 4.5 → 1.0 s; phone 4 Mbit/s 12.6 → 4.4 s. A brisk wheel scroll straight after opening showed 2 late frames on the fast connections. On the slow ones many frames arrive late (about 240–290 over a 4 s scroll), and the story then shows the nearest frame already downloaded (on average 4–8 atlases away) until the gaps fill. The screen is not shown for reduced motion.
